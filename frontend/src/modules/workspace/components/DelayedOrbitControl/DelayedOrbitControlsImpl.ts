...
class OrbitControls extends EventDispatcher {
  ...
  constructor(
    ...
    function onPointerMove(event: PointerEvent) {
      ...
      if (event.pointerType === "touch") {
        // TODO: Implement delay behaviour for touch devices as well
        onTouchMove(event);
      } else {
        onMouseMove(event);
      }
    ...
    function onMouseDown(event: MouseEvent) {
      ...
      scope.moving = new Vector2(event.clientX, event.clientY);
    ...
    function onMouseMove(event: MouseEvent) {
      ...
      if (scope.moving !== null) {
        const dx = event.clientX - scope.moving.x;
        const dy = event.clientY - scope.moving.y;
        if (dx * dx + dy * dy >= 4) {
          // 2 pixels
          scope.moving = null;
...
export { OrbitControls as DelayedOrbitControlsImpl, MapControls };
