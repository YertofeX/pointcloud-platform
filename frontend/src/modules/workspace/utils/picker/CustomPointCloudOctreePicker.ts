...
import { ClipMode, PointCloudMaterial, PointColorType } from "potree-core";
...
export class CustomPointCloudOctreePicker {
  ...
  pick(
    ...
    const pickState = this.pickState
      ? this.pickState
      : (this.pickState = CustomPointCloudOctreePicker.getPickState());
    ...
    CustomPointCloudOctreePicker.prepareRender(
      renderer,
      x,
      y,
      pickWndSize,
    ...
    const renderedNodes = CustomPointCloudOctreePicker.render(
      renderer,
      camera,
      pickMaterial,
      octrees,
  ...
  private static prepareRender(
    ...
    y: number,
    pickWndSize: number,
    pickMaterial: CustomPointCloudMaterial,
  ...
  private static render(
    ...
    for (const octree of octrees) {
      ...
      const nodes = CustomPointCloudOctreePicker.nodesOnRay(octree, ray);
      if (!nodes.length) {
        continue;
      }
      ...
      pickState.scene.children = CustomPointCloudOctreePicker.createTempNodes(
        octree,
        nodes,
        pickMaterial,
  ...
  private static nodesOnRay(octree: PointCloudOctree, ray: Ray): PointCloudOctreeNode[] {
    ...
    for (const node of octree.visibleNodes) {
      const sphere = CustomPointCloudOctreePicker.helperSphere
  ...
  private static createTempNodes(
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      tempNode.onBeforeRender = CustomPointCloudMaterial.makeOnBeforeRender(
        octree,
        node,
        nodeIndex
      );
  ...
  private static getPickState() {
    ...
    const material = new CustomPointCloudMaterial();
    material.pointColorType = PointColorType.POINT_INDEX;

    return {
      renderTarget: CustomPointCloudOctreePicker.makePickRenderTarget(),
      material: material,
      scene: scene,
    };
  }
