...
import { IPointCloudTreeNode, PointCloudOctree } from "potree-core";
import { PointCloudOctreeNode } from "potree-core";
function byLevelAndIndex(a: IPointCloudTreeNode, b: IPointCloudTreeNode) {
  const na = a.name;
  const nb = b.name;
...
const DEFAULT_CLASSIFICATION: IClassification = {
...
import { ClipMode, IClipBox } from "potree-core";
import { PointColorType, PointOpacityType, PointShape, PointSizeType, TreeType } from "potree-core";
import { SPECTRAL } from "potree-core";
import {
  generateClassificationTexture,
  generateDataTexture,
  generateGradientTexture,
} from "potree-core";
import { IClassification, IGradient, IUniform } from "potree-core";
enum ColorEncoding {
  LINEAR = 0,
  SRGB = 1,
}
...
//@ts-ignore
import * as pointcloudshadersvs from "./pointcloud.vs";
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
//@ts-ignore
import * as pointcloudshadersfs from "./pointcloud.fs";

const VertShader = pointcloudshadersvs.default;
const FragShader = pointcloudshadersfs.default;
...
export class CustomPointCloudMaterial extends RawShaderMaterial {
  ...
  updateMaterial(
    ...
    this.octreeSize = octree.pcoGeometry.boundingBox.getSize(CustomPointCloudMaterial.helperVec3).x;
  ...
  static makeOnBeforeRender(
    ...
    return (
      ...
      const pointCloudMaterial = material as CustomPointCloudMaterial;
